...
public class UserRepositoryImp implements UserRepository {
    ...
    public UserEntity onlogin(String email, String password) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            String queryStr = "SELECT p FROM UserEntity p WHERE p.email = :email";
    ...
    public long getCountofName(String name) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            String queryStr = "SELECT COUNT(p) FROM UserEntity p WHERE p.name = :name";
            Query query = entityManager.createQuery(queryStr);
    ...
    public long getCountofEmail(String email) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            String queryStr = "SELECT COUNT(p) FROM UserEntity p WHERE p.email = :email";
            Query query = entityManager.createQuery(queryStr);
            query.setParameter("email", email);
    ...
    public long getCountofNumber(String phoneNumber) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            String queryStr = "SELECT COUNT(p) FROM UserEntity p WHERE p.phoneNumber = :phoneNumber";
            Query query = entityManager.createQuery(queryStr);
    ...
    public long getAlternateEmail(String alternateemail) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            String Quesrystr = "SELECT COUNT(p) FROM UserEntity p WHERE p.alternateemail = :alternateemail";
    ...
    public long getAlternatePhone(String alternatephone) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            String QueryStr = "SELECT COUNT(p) FROM UserEntity p WHERE p.alternatephone = :alternatephone";
            Query query = entityManager.createQuery(QueryStr);
